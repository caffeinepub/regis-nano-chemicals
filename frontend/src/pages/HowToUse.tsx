import {
  Droplets, Wind, SprayCan, RotateCw, Sparkles,
  Search, Pipette, Move, Clock, Car,
  Trash2, CheckCircle, ArrowRight
} from 'lucide-react';
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative text-center">
          ...
          <p className="text-white/70 font-inter text-base mt-4 max-w-xl mx-auto fade-in-delay-3">
            Follow these simple step-by-step instructions to get the best results from every REGIS NANO product.
          </p>
        </div>
      </div>
          ...
          <div className="space-y-16">
            {instructions.map((product, productIndex) => (
              <FadeSection key={product.id}>
                <div className={`rounded-3xl overflow-hidden shadow-card border border-brand-gray-mid ${
                  ...
                  >
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-poppins font-800 text-lg ${
                      product.color === 'blue'
                    ...
                    <h2 className={`font-poppins font-800 text-2xl sm:text-3xl ${
                      product.color === 'blue' ? 'text-white' : 'text-brand-blue-dark'
                    }`}>
                      {product.name}
                    </h2>
                  </div>
                  ...
                  <div className="p-8">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                      {product.steps.map((step, stepIndex) => {
                        ...
                        return (
                          <div key={stepIndex} className="relative flex flex-col items-center text-center group">
                            {/* Connector line */}
                            {stepIndex < product.steps.length - 1 && (
                              <div className="hidden lg:block absolute top-8 left-1/2 w-full h-0.5 bg-brand-gray-mid z-0" />
                            )}
                            {/* Step circle */}
                            <div className={`relative z-10 w-16 h-16 rounded-2xl flex items-center justify-center mb-4 shadow-card group-hover:scale-110 transition-transform duration-200 ${
                              product.color === 'blue'
                            ...
                            {/* Step number badge */}
                            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-poppins font-700 mb-3 ${
                              product.color === 'blue'
                                ? 'bg-brand-yellow text-brand-blue-dark'
                                : 'bg-brand-blue text-white'
                            }`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <div key={tip} className="bg-white rounded-2xl p-6 shadow-card border border-brand-gray-mid hover:shadow-card-hover transition-shadow">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h3 className="font-poppins font-700 text-brand-blue text-base">{tip}</h3>
                </div>
