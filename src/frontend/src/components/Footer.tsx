...
export default function Footer() {
  ...
  const appId = encodeURIComponent(
    typeof window !== "undefined" ? window.location.hostname : "pearl-shine",
  );
    ...
    <footer className="bg-brand-blue-dark text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm font-inter text-white/70">
              <li>
                ...
                >
                  regisnanochemicals2020@gmail.com
                </a>
              </li>
              <li>
        ...
        <div className="mt-12 pt-6 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-white/50 font-inter">
          <p>© {year} Pearl Shine. All rights reserved.</p>
          <p className="flex items-center gap-1">
            Built with{" "}
            <Heart className="w-3 h-3 text-brand-yellow fill-brand-yellow mx-0.5" />{" "}
            using{" "}
            <a
